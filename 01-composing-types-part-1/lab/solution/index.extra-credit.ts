...
type ApiResponse<StatusCodeType extends 200 | 404 | 500, DataType = null> = {
  statusCode: StatusCodeType;
  data: DataType;
  error: StatusCodeType extends 200 ? null : string;
};

const photoResponse = api.getPhotoById(1);

if (photoResponse.statusCode === 200) {
  const photo: ApiResponse<200, Photo> = photoResponse;
  ...
  const photo: ApiResponse<404> = photoResponse;
  ...
  const photo: ApiResponse<500> = photoResponse;
